...
*/
typedef double (*kmeans_distance_method) (const Pointer a, const Pointer b, size_t l);
...
* @param l the dimension of the point we are dealing with
*/
typedef void (*kmeans_centroid_method) (const Pointer * objs, const int * clusters, size_t num_objs, int cluster, Pointer centroid, size_t l);
...
typedef struct kmeans_config
{
	...
	/* Dimension of the vectors we are calculating, length of the objs */
	size_t l;

	/* Number of means we are calculating, length of the centers array */
	unsigned int k;
