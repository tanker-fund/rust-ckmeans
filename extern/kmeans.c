...
update_r(kmeans_config *config)
{
	...
	for (i = 0; i < config->num_objs; i++)
	{
		...
		*/
		...
		curr_distance = (config->distance_method)(obj, config->centers[0], config->l);
		curr_cluster = 0;

		/* Check all other cluster centers and find the nearest */
		for (cluster = 1; cluster < config->k; cluster++)
		{
			distance = (config->distance_method)(obj, config->centers[cluster], config->l);
...
update_means(kmeans_config *config)
{
	...
	for (i = 0; i < config->k; i++)
	{
		...
		(config->centroid_method)(config->objs, config->clusters, config->num_objs, i, config->centers[i], config->l);
	}
}
